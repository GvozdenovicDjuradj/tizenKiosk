...
class EventCheckInScreen extends ComponentWithNavigation<Props> {
  public render() {
    ...
    return (
      ...
      >
        ...
        <Header
          style={{ marginTop: 0 }}
          text="Enter your details so we can find your booking"
          template={template}
        />
        <Content />
...
const isInvalidSelector = (state: RootState) => {
  const nodata = (
    Boolean(state.eventCheckIn.email) === false &&
    Boolean(state.eventCheckIn.bookingRef) === false
  )
  if (nodata) {
    return true
  }
  return !selectors.isScreenValid(AppScreens.EVENT_CHECK_IN)(state)
}
...
const mapDispatchToProps = (dispatch: Dispatch<Action>): PropsFromDispatch => ({
  goBackExtra: () => dispatch({ type: KIOSK.EVENT_CHECK_IN.RESET }),
  goNext: () => dispatch({ type: KIOSK.EVENT_CHECK_IN.REQUEST })
})

export default connect(mapStateToProps, mapDispatchToProps)(EventCheckInScreen)
